...
mod token_program {
    ...
    // Add multisig mint creation
    pub fn create_multisig_mint(ctx: Context<CreateMultisigMint>, decimals: u8) -> Result<()> {
        msg!("Multisig token mint created: {}", ctx.accounts.mint.key());
        Ok(())
    }
    ...
    pub fn multisig_mint_tokens(ctx: Context<MultisigMintTokens>, amount: u64) -> Result<()> {
        let cpi_accounts = token::MintTo {
            ...
            authority: ctx.accounts.multisig_authority.to_account_info(),
        };
        ...
        msg!("Multisig minted {} tokens to {}", amount, ctx.accounts.token_account.key());
        Ok(())
    }
...
// Add multisig mint creation struct
#[derive(Accounts)]
pub struct CreateMultisigMint<'info> {
    #[account(
        ...
        seeds = [b"multisig_mint", payer.key().as_ref()],
        bump
    )]
    pub mint: Account<'info, Mint>,
    
    /// CHECK: This will be a multisig account created externally
...
// Add multisig mint tokens struct
#[derive(Accounts)]
pub struct MultisigMintTokens<'info> {
    ...
    /// CHECK: This is a multisig account that must have been created externally
    /// and have the required signatures attached to the transaction
    pub multisig_authority: UncheckedAccount<'info>,
